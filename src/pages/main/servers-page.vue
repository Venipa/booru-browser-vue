<template>
  <div class="flex w-full bg-black bg-opacity-20 backdrop-filter backdrop-blur-lg">
    <div class="server-list flex flex-col flex-1 gap-2 py-2">
      <div class="bg-gray-800 text-gray-50 bg-opacity-30 backdrop-filter backdrop-blur-lg px-4 py-3 rounded-lg shadow mx-2 flex items-center gap-3 cursor-pointer select-none" v-for="s in servers" :key="s.name">
        <CheckCircleIcon class="h-8 w-8 text-white bg-green-500 rounded-full p-1.5" v-if="s.active"></CheckCircleIcon>
